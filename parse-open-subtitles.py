...
Used for ignoring dirs with year numbers instead of genres
'''
def is_number(s):
    try:
        float(s)
...
def extractTokenizedPhrases(xmlFilePath, dataDirFilePath):
    ...
    movie_genre = xmlFilePath.split('/')[5]
    movie_title = '-'.join(xmlFilePath.split('_')[3:]).replace('.xml', '')
    file_path = dataDirFilePath + movie_genre + '_' + movie_title + '_' + raw_file
